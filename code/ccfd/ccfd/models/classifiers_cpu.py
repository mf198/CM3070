...
def train_xgboost(X_train: pd.DataFrame, y_train: pd.Series) -> xgb.XGBClassifier:
    ...
    model = xgb.XGBClassifier(use_label_encoder=False, eval_metric='logloss', scale_pos_weight=5, random_state=42)
    model.fit(X_train, y_train)
    return model
...
def evaluate_model_cpu(model, X_test: pd.DataFrame, y_test: pd.Series, use_gpu=False) -> Dict[str, float]:
    ...
    #cf = confusion_matrix(y_test, y_proba)
    #print(cf)            


    # Compute evaluation metrics (cuML for GPU, scikit-learn for CPU)
    if use_gpu:
        return {
            "accuracy": cu_accuracy_score(y_test, y_proba),
            "precision": precision_score(y_test, y_pred),
            "recall": recall_score(y_test, y_pred),            
            "f1_score": f1_score(y_test, y_pred),
            "roc_auc": cu_roc_auc_score(y_test, y_proba),
        }
    else:
        return {
